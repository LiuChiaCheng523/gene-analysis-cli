...
usage <- function() {
  cat(
    paste0(
      ...
      "  fdr_cutoff            Used only in output file naming. Default: 0.15\n",
      "  gene_file             Default: overlap/<project_name_overlap>/overlap_gene_name_pairwise_union.csv\n",
      "  universe_file         Default: overlap/<project_name_overlap>/twas_detected_gene_name.csv\n\n",
      "Examples:\n",
...
gene_file <- if (!is.na(args$gene_file) && args$gene_file != "") {
  args$gene_file
} else {
  file.path(overlap_input_folder, "overlap_gene_name_pairwise_union.csv")
}

universe_file <- if (!is.na(args$universe_file) && args$universe_file != "") {
  args$universe_file
} else {
  file.path(overlap_input_folder, "twas_detected_gene_name.csv")
}
...
if (nrow(ego_union_df) > 0) {
  ...
  p_go <- ggplot(
    ...
    labs(
      ...
      title = paste0("GO Biological Process Enrichment of ", project_name_overlap, " Candidate Genes Top20")
    ) +
  ...
  ggsave(
    file.path(gobp_output_folder, paste0(project_name_overlap, "_GOBP_ORA_FDR", fdr_tag, ".png")),
    plot = p_go,
    width = 15,
    height = 8,
    dpi = 600
...
write.csv(
  ...
  file.path(gobp_output_folder, paste0(project_name_overlap, "_GOBP_ORA_FDR", fdr_tag, "_summary.csv")),
  row.names = FALSE
)

write.csv(
  as.data.frame(ego_bp) %>% arrange(desc(RichFactor)),
  file.path(gobp_output_folder, paste0(project_name_overlap, "_GOBP_ORA_FDR", fdr_tag, "_full.csv")),
  row.names = FALSE
)
...
if (nrow(ekegg_union_df) > 0) {
  ...
  p_kegg <- ggplot(
    ...
    labs(
      x = "Rich Factor",
      y = "KEGG Pathway",
      title = paste0("KEGG Pathway Enrichment of ", project_name_overlap, " Candidate Genes Top20")
    ) +
    theme_bw(base_size = 12) +
    theme(
      plot.title = element_text(hjust = 0.5, face = "bold"),
  ...
  ggsave(
    file.path(kegg_output_folder, paste0(project_name_overlap, "_KEGG_ORA_FDR", fdr_tag, ".png")),
    plot = p_kegg,
    width = 15,
    height = 8,
    dpi = 600
...
write.csv(
  ...
  file.path(kegg_output_folder, paste0(project_name_overlap, "_KEGG_ORA_FDR", fdr_tag, "_summary.csv")),
  row.names = FALSE
)

write.csv(
  as.data.frame(ekegg) %>% arrange(desc(RichFactor)),
  file.path(kegg_output_folder, paste0(project_name_overlap, "_KEGG_ORA_FDR", fdr_tag, "_full.csv")),
  row.names = FALSE
)
